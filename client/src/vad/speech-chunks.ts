...
import { VadDetector } from "./voice-activity-detector";
...
export class SpeechChunks {
  ...
  private static readonly END_THRESHOLD = 0.6;
  ...
  private onSpeechStart: OnSpeechStart;
  private onSpeechEnd: OnSpeechEnd;

  constructor(onSpeechStart: OnSpeechStart, onSpeechEnd: OnSpeechEnd) {
    this.chunks = [];
  ...
  private async processAudioData(audioData: Float32Array): Promise<void> {
    try {
      const result = await this.vadDetector.run(audioData, false);
  ...
  getBlob(combinedAudio: Float32Array): Blob {
